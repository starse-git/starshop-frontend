...
import { Product, ProductImage, RecommendedProduct } from "@/types/products";
...
const ProductPage = () => {
  ...
  const [tempSelectedRecommendProducts, setTempSelectedRecommendProducts] =
    useState<{ product_id: number; name: string; image_url: string }[]>([]); // State to store temporary selected recommend products
  ...
  const { data: recommendProducts } = useGetRecommendProductsToChoose(
    productId || 0
  ); // Custom hook to fetch recommend products

  const { data: recommendProductsData } = useGetRecommendProducts(
    productId || 0
  ); // Custom hook to fetch recommend products
  ...
  useEffect(() => {
    const productItem = product?.data?.data;
    const categoryList = categories?.data;
    const brandList = brands?.data;

    if (productItem && categoryList && brandList) {
      form.reset({
        shippingType: productItem.shipping_fees === null ? "free" : "fixed",
        name: productItem.name,
        category: productItem.category_id.toString(),
        tax: productItem.tax.toString(),
        brand: productItem.brand_id.toString(),
        price: productItem.wholesale_price.toString(),
        salePrice: productItem.sale_price.toString(),
        description: productItem.description,
        regularPercentage: productItem.regular_percentage?.toString(),
        regularDiscountPrice: productItem.regular_discount?.toString(),
        stock: productItem.stock_quantity.toString(),
        safeStock: productItem.safe_stock_quantity.toString(),
        shippingFee: productItem.shipping_fees?.toString() || "",
        images: productItem.product_images.map((image: ProductImage) => ({
    ...
    if (recommendProductsData) {
      setSelectedRecommendProducts(
        recommendProductsData.data?.data?.map((item: RecommendedProduct) => ({
          product_id: item.recommended_product_id,
          name: item.recommended_product?.name ?? "",
          image_url: item.recommended_product?.product_images[0]?.image_url
            ? getPublicUrl(
                item.recommended_product.product_images[0]?.image_url
              )
  ...
  }, [product, categories, brands, recommendProductsData, form]);
      ...
      const deleteAndUploadAndSaveOrder = async () => {
        try {
          ...
          if (imagesToDelete.length > 0) {
            deleteFiles({ files: imagesToDelete as string[] });
            deleteImageRecords({
          ...
          for (let i = 0; i < finalImages.length; i++) {
            ...
            if (!img.del_flg && img.file instanceof File) {
              const result = await uploadProductImage(img.file, productId || 0);
